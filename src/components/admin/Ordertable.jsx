    ...
    <div className="bg-white border border-black/[0.07] rounded-2xl overflow-hidden">
      ...
      {orders.map((order) => (
        <div
          ...
          style={{ animation: `fadeSlideUp 0.4s ease 0s both` }}
        >
          <div className="text-yellow-500 text-xs font-semibold flex items-center">
            {order.id}
          </div>
          <div className="flex items-center gap-2.5">
            <div
              className="w-7 h-7 rounded-lg flex-shrink-0 flex items-center justify-center text-yellow-400 text-[10px] font-bold border border-yellow-500/20"
              style={{
          ...
          <div className="flex items-center">
            <span
              className={`text-[10px] font-semibold px-2.5 py-1 rounded-full flex items-center gap-1.5 ${statusStyles[order.status]}`}
            >
              <span
