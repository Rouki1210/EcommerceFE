    ...
    <div className="animate-[fadeSlideUp_0.5s_ease_both]">
      ...
      {loading ? (
        ...
        <div className="bg-red-50 rounded-2xl p-8 text-center border border-red-200">
          ...
          <button
            onClick={loadUsers}
            className="mt-3 bg-red-500 border-none text-white px-4 py-2 rounded-lg cursor-pointer text-xs"
          >
            Retry
          </button>
        </div>
        ...
        <div className="bg-white border border-black/[0.07] rounded-2xl overflow-hidden">
          {/* Table header */}
          <div
            className="grid px-6 py-4 border-b border-black/5 bg-slate-50"
          ...
          >
            {[
              ...
              <div
                key={h}
                className="text-slate-700 text-[10px] tracking-[1.5px] font-semibold uppercase"
              >
                {h}
              </div>
            ))}
          ...
          {filtered.map((user, i) => (
            ...
            >
              <div className="flex items-center gap-2.5">
                <div
                  className="w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center text-yellow-400 text-xs font-bold border border-yellow-500/20"
                  style={{
              ...
              <div className="flex items-center">
                <span
                  className={`text-[10px] font-semibold px-2.5 py-1 rounded-full ${roleStyle[user.role] || "bg-slate-100 text-slate-500"}`}
                >
                  {user.role || "Customer"}
                </span>
              </div>
              ...
              <div className="text-yellow-500 text-xs font-bold flex items-center">
                {user.spent || "$0"}
              </div>
              <div className="flex items-center">
                <span
                ...
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${user.active ? "bg-emerald-400" : "bg-slate-400"}`}
                  />
