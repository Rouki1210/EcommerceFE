...
const inputCls = "w-full bg-slate-50 border border-black/[0.08] rounded-lg px-3 py-2 text-slate-900 text-xs outline-none focus:border-yellow-400 transition-colors";
const labelCls = "text-slate-400 text-[9px] tracking-[1.5px] block mb-1.5 font-semibold uppercase";

export default function ProductModal({ product, onClose, onSave }) {
    const [form, setForm] = useState({
        name:"", category:"", gender:"women", price:"", badge:"", variant:"",
        description:"", material:"", care:"", sizes:[], stock:"",
        sizeChart:{ headers:["Size","Chest (cm)","Waist (cm)","Length (cm)"], rows:[] },
        image:null, imageFile:null, ...product
    });
            ...
            >
                ...
                >
                    ...
                    <div className="px-5 pt-5 pb-0 border-b border-black/[0.06] flex justify-between items-center flex-wrap gap-2.5">
                        <div>
                            <div className="text-slate-900 text-lg font-extrabold">{form.id ? "Edit Product" : "Add Product"}</div>
                            <div className="text-slate-400 text-[11px] mt-0.5">Fill in all product information</div>
                        </div>
                            ...
                            {tabs.map(t => (
                                <motion.button
                                    ...
                                    className={`px-4 py-1.5 rounded-lg border-none cursor-pointer text-xs capitalize transition-all ${
                                        tab === t ? "bg-white text-slate-900 font-bold shadow-sm" : "bg-transparent text-slate-400 font-medium"
                                    }`}
                    ...
                    <div className="p-5 overflow-y-auto flex-1">
                        <AnimatePresence mode="wait">
                            ...
                            >
                                {/* BASIC TAB */}
                                {tab === "basic" && (
                                    <div className="grid gap-5" style={{ gridTemplateColumns: "min(200px,35%) 1fr" }}>
                                        {/* Image upload */}
                                        <div>
                                            ...
                                            <motion.div
                                                ...
                                                className={`border-2 border-dashed rounded-xl cursor-pointer overflow-hidden flex items-center justify-center transition-all ${
                                                    dragging ? "border-yellow-400 bg-yellow-500/[0.04]" : "border-black/10 bg-slate-50"
                                                } ${preview ? "" : "h-40"}`}
                                            >
                                                {preview ? (
                                                    <div className="relative w-full">
                                                        <img src={preview} alt="preview" className="w-full h-40 object-cover block" />
                                                    ...
                                                    <div className="text-center p-4">
                                                        ...
                                                        <div className="text-slate-500 text-[11px] font-semibold">Click to select</div>
                                                        <div className="text-slate-400 text-[10px] mt-1">or drag & drop</div>
                                                        <div className="text-slate-300 text-[9px] mt-0.5">PNG · JPG · WEBP</div>
                                                    </div>
                                                )}
                                            </motion.div>
                                        ...
                                        <div className="grid grid-cols-2 gap-3.5">
                                            <div className="col-span-2">
                                                ...
                                                <input value={form.name} onChange={e => set("name", e.target.value)} placeholder="e.g. Merino Wool Sweater" className={inputCls} />
                                            </div>
                                            <div>
                                                <label className={labelCls}>Category *</label>
                                                <input value={form.category} onChange={e => set("category", e.target.value)} placeholder="e.g. Knitwear" className={inputCls} />
                                            </div>
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <input value={form.price} onChange={e => set("price", e.target.value)} placeholder="e.g. 128" type="number" className={inputCls} />
                                            </div>
                                            <div>
                                                <label className={labelCls}>Stock *</label>
                                                <input value={form.stock} onChange={e => set("stock", e.target.value)} placeholder="e.g. 50" type="number" className={inputCls} />
                                            </div>
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <input value={form.variant} onChange={e => set("variant", e.target.value)} placeholder="e.g. Ivory" className={inputCls} />
                                            </div>
                                            <div className="col-span-2">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="col-span-2">
                                            ...
                                            <textarea value={form.description} onChange={e => set("description", e.target.value)}
                                                      placeholder="A classic merino wool sweater..." rows={4}
                                                      className={inputCls + " resize-y leading-relaxed font-[inherit]"} />
                                        </div>
                                        <div>
                                            <label className={labelCls}>Material</label>
                                            <input value={form.material} onChange={e => set("material", e.target.value)} placeholder="e.g. 100% Merino Wool" className={inputCls} />
                                        </div>
                                        <div>
                                            <label className={labelCls}>Care Instructions</label>
                                            <input value={form.care} onChange={e => set("care", e.target.value)} placeholder="e.g. Hand wash cold" className={inputCls} />
                                        </div>
                                        {(form.name || form.price) && (
                                            <div className="col-span-2">
                                                <label className={labelCls}>Preview</label>
                                                <div className="bg-slate-50 rounded-xl p-4 border border-black/[0.06] flex gap-4 items-start">
                                                    {preview && <img src={preview} alt="" className="w-16 h-16 object-cover rounded-lg" />}
                                                    <div>
                                                        <div className="text-slate-900 font-bold text-sm">{form.name || "—"}</div>
                                                        <div className="text-slate-500 text-[11px] mt-0.5">{form.category} · {form.gender} · {form.variant}</div>
                                                        {form.badge && <span className="inline-block mt-1 bg-yellow-500/15 text-yellow-700 text-[10px] font-bold px-2 py-0.5 rounded-full">{form.badge}</span>}
                                                        <div className="text-yellow-400 font-extrabold text-base mt-1.5">${form.price}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                                    ...
                                    <div>
                                        <div className="flex justify-between items-center mb-3">
                                            <div>
                                                ...
                                                <div className="text-slate-500 text-[11px]">Headers: {form.sizeChart.headers.join(" · ")}</div>
                                            </div>
                                            ...
                                            >+ Add Row</motion.button>
                                        </div>
                                        <div className="rounded-xl overflow-hidden border border-black/[0.07]">
                                            <div className="bg-slate-50 border-b border-black/[0.06]"
                                                 style={{ display: "grid", gridTemplateColumns: `repeat(${form.sizeChart.headers.length}, 1fr) 36px` }}>
                                                {form.sizeChart.headers.map((h, i) => (
                                                    <div key={i} className="px-3 py-2.5 text-slate-500 text-[10px] font-bold tracking-wide">{h}</div>
                                                ))}
                                                <div />
                                            </div>
                                            {form.sizeChart.rows.length === 0 && (
                                                <div className="p-6 text-center text-slate-300 text-xs">No rows yet — click "+ Add Row"</div>
                                            )}
                                            <AnimatePresence>
                                                {form.sizeChart.rows.map((row, ri) => (
                                                    ...
                                                    >
                                                        {row.map((cell, ci) => (
                                                            <input key={ci} value={cell} onChange={e => updateSizeRow(ri, ci, e.target.value)}
                                                        ...
                                                        <button onClick={() => removeSizeRow(ri)}
                                                                className="border-none bg-transparent cursor-pointer text-red-400 text-sm hover:text-red-500 transition-colors">✕</button>
                                                    </motion.div>
                                                ))}
                                            </AnimatePresence>
                                        </div>
                    ...
                    <div className="px-5 py-3.5 border-t border-black/[0.06] flex justify-between items-center flex-wrap gap-2.5 bg-slate-50 rounded-b-2xl">
                        ...
                        <div className="flex gap-2.5">
                            ...
                            <motion.button
                                whileHover={{ scale: 1.03, boxShadow: "0 6px 20px rgba(234,179,8,0.4)" }}
                                whileTap={{ scale: 0.97 }}
                            ...
                            >{form.id ? "Save Changes" : "Add Product"}</motion.button>
                        </div>
                    </div>
                </motion.div>
            </motion.div>
